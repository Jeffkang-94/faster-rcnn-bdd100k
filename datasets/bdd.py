...
def get_ground_truths(train_img_path_list, anno_data, dt=False):
    if dt:
        domain_transform = T.DomainTransfer()
        transform_list = tf.Compose([
    ...
    for i in tqdm(range(len(train_img_path_list))):
        if anno_data[i]["attributes"]['timeofday'] =='night' or anno_data[i]["attributes"]['timeofday'] =='daytime':
        #if anno_data[i]["attributes"]['timeofday'] =='daytime':
            ...
            # domain transfer #
        """if anno_data[i]["attributes"]['timeofday'] =='daytime' and dt:
            #day_image = Image.open(train_img_path_list[i]).convert('RGB')
            
            day_image = pil_loader(train_img_path_list[i])
            day_image = transform_list(day_image)
            day_image = day_image.cuda()
            fake_night_image = domain_transform(day_image.unsqueeze(0)).squeeze(0)
            path = os.path.basename(train_img_path_list[i])
            fake_night_path = '/mnt2/datasets/bdd100k/images/100k/fake_night_noTV_noRefine/{}'.format(path)
            save_image(fake_night_image,fake_night_path, normalize=True)
            total_img_path.append(fake_night_path)
